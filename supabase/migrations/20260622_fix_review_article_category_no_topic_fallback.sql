-- review_article_category_safe must only ever set a REAL category (one of the 5)
-- or NULL. The previous version fell back to the article's `topic`, which tagged
-- general Daily Wrap articles as "India"/"India Business" and dropped them out of
-- both the wrap pool (category IS NULL) and the category tabs.
...
begin
  ...
  if p_action = 'approve' then
    ...
    if v_count >= 10 then
      raise exception 'Daily approval limit reached for %', coalesce(v_category, 'Daily Wrap');
    end if;

...
function renderTopMeta(): string {
  ...
  return `<table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="background:#3979ff"><tr><td style="padding:8px 24px;color:#ffffff;font:12px/22px Roboto,Arial,sans-serif">From the Dailymattr Team</td><td style="padding:8px 24px;color:#ffffff;font:12px/22px Roboto,Arial,sans-serif;text-align:right">${today}</td></tr></table>`;
}
...
async function renderDigest(items: DigestItem[], sub: Subscriber, topic: string): Promise<string> {
  ...
  const intro = topic === "case-study-pool"
    ? "Here is today's Dailymattr case study, picked from the approved case-study pool."
    : topic === "all-topics"
    ? "Here are the approved stories across Dailymattr's topic newsletters, minus the noise."
    : `Here is today's approved ${topicLabel(topic)} edition from Dailymattr, written for a fast and useful read.`;
